...
A total of `r summarise(techno_data, n())` scaled pieces were analyzed, of which `r round(techno_data %>% filter(Chronology == "Gravettian") %>% summarise(n = n()) %>% mutate(n = (n / 139)*100), digits = 2)`% come from Gravettian, and `r round(techno_data %>% filter(Chronology == "Solutrean") %>% summarise(n = n()) %>% mutate(n = (n / 139)*100), digits = 2)`% from Solutrean levels, as shown in (Table \@ref(tab:assemblage)).The assemblage was divided into 4 main chronological groups: Magdalenian, Solutrean, Proto-Solutrean, and Gravettian.

As previously mentioned, as far as the technological data is concerned, one of our main objectives was to characterize and understand the choice of blanks involved in bipolar technology. We did not include the metric data, as most of the data from the original blanks are mostly gone due to reduction and use. Overall, the technological analysis revealed some trends that lasted throughout the Upper Paleolithic, in agreement with the general patterns of lithic technology in the site (see above).
...
Across all techno-complexes, blanks used were either flakes or unclassifiable fragments, but the choice seems to be different for each raw material. For quartz, in the Gravettian assemblage, the blank types are almost equally split between flakes and unclassifiable pieces (Table \@ref(tab:technogravettian)). In other periods, flakes were the preferred type of blank (Tables \@ref(tab:technosolutrean), \@ref(tab:technoproto) and \@ref(tab:technomagdalenian)). Regarding chert, in every occupation flakes dominate the assemblages, followed by a reduced number (n = `r techno_data %>% filter(RawMaterial == "Chert" & Blank == "Non_identifiable") %>% summarise(n())`) of unclassifiable blanks. Chalcedony blanks are exclusively flakes.
...
Gravettian <- select(Gravettian, " ", Chert, Quartz, Total) #to remove chalcedony
...
Magdalenian <- select(Magdalenian, " ", Chert, Quartz, Total) #to remove chalcedony
...
Technological and morphological characteristics of the flake blanks present very similar patterns across time and amongst raw materials. The blanks sought after would have the following characteristics: straight profile, parallel edges, no cortical surfacem and trapezoidal or triangular shaped cross-sections.

Other characteristics of the assemblage are the low frequency of retouch (n = `r techno_data %>% filter(RetouchedBlank == "Yes") %>% summarise(n())`), fire alterations (n = `r techno_data %>% filter(Fire != "No_Traces") %>% summarise(n())`), and of the original striking platforms (n = `r techno_data %>% filter(ButtType != "Absent") %>% summarise(n())`). Still, when present, retouch was located in the lateral part of the artifacts, similar to what would define a side-scraper. In a very small number of cases when striking platforms were detected, these were mostly flat. The absence of the original blank platforms is to be expected in this type of artifact, mostly due to the functional use of the pieces, and consequent removal of the platform, rather than an actual choice. The large absence of platforms may, on the other hand, be the result of the use of the technological axis as the main functional axis. In fact, when identifiable, technological and typological axes coincide in `r inline_perc(techno_data, AxisCoincident, "Yes")`% of the cases. The longitudinal section shows a large variability of shapes, independent of raw material or chronology. Similarly, the dorsal pattern of previous removals was rarely identified, although this, like with the case of the platform absence, might occur due to the functional stigmas and be dependent on the the intensity of use for each artifact.

The patterns of use and rotation of damaged platforms seem to be the same across all chronologies. In every assemblage quartz pieces were exclusively used in one single axis, exhibiting only two damaged platforms. On the other end, some (3,38%) chert and chalcedony artifacts exhibit multiple functional axis, with three or four damaged platforms (Figure \@ref(fig:nplatforms)). This seems to indicate different strategies of curation for coarse and fine grain raw materials.

\newline

```{r nplatforms, fig.cap=""}
...
morpho_data <- morpho_data %>% 
  ...
  select(RawMaterial, Chronology,
         ScarDistribution_PlatformA, ScarDistribution_PlatformB,
         ScarArrangement_PlatformA,ScarArrangement_PlatformB,
         ScarExtention_PlatformA,ScarExtention_PlatformB,
         ScarFacialDistribution_PlatformA, ScarFacialDistribution_PlatformB,
         ScarEdgeDelineation_PlatformA, ScarEdgeDelineation_PlatformB,
         Angle_PlatformA, Angle_PlatformB)


var_list <- c("DamageDegree", "ScarShape", "ScarDistribution", "ScarArrangement", "ScarExtension", "ScarFacialDistribution")

# Filter by chronology
filter <- split(morpho_data_2plat, morpho_data$Chronology)
...
# Recode based on 10% frequency rule
condense(morpho_data)


# Join attributes
morpho_data_2plat <- morpho_data %>% 
  mutate(DamageDegree = str_c(DamageDegree_PlatformA,DamageDegree_PlatformB, sep = "/")) %>% 
  mutate(ScarShape = str_c(ScarShape_PlatformA,ScarShape_PlatformB, sep = "/")) %>%
  mutate(ScarDistribution = str_c(ScarDistribution_PlatformA,ScarDistribution_PlatformB, sep = "/")) %>%
  mutate(ScarArrangement = str_c(ScarArrangement_PlatformA,ScarArrangement_PlatformB, sep = "/")) %>%
  mutate(ScarExtension = str_c(ScarExtention_PlatformA,ScarExtention_PlatformB, sep = "/")) %>%
  mutate(ScarFacialDistribution = str_c(ScarFacialDistribution_PlatformA,ScarFacialDistribution_PlatformB, sep = "/")) %>%
  mutate(Scar_EdgeDelineation = str_c(ScarEdgeDelineation_PlatformA,ScarEdgeDelineation_PlatformB, sep = "/")) %>%
  mutate(Angle = str_c(Angle_PlatformA,Angle_PlatformB, sep = "/"))
