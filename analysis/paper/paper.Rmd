...
In scaled pieces, metrics are a difficult set of variables to clearly show as results. These attributes are impacted by both the initial blank size as well as by the functional use and reduction. Consideranging the typological axes (see Table \@ref(tab:tech_attributes)) for definition) length ranging from `r min(techno_data$AxisLength, na.rm = TRUE)` mm to `r max(techno_data$AxisLength, na.rm = TRUE)` mm, width ranging from `r min(techno_data$AxisWidth, na.rm = TRUE)` mm to `r max(techno_data$AxisWidth, na.rm = TRUE)` mm and thickness ranging from `r min(techno_data$Thickness, na.rm = TRUE)` mm to `r max(techno_data$Thickness, na.rm = TRUE)` mm. In general, mean values tend to be similiar between raw materials, with some differences occuring within the Proto-Solutrean and Magdalenian assemblages, most certainly as a result of the small samples analysed for each of these periods (Figure \@ref(fig:meanplots)).

```{r meanplots, echo = FALSE, fig.cap="Barplots of means for Length, Width and Thickness of scaled pieces by raw material and across the four chronological phases. Error bars represent standard deviations."}
