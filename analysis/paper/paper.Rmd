...
With few exceptions (platform angle and facial scar distribution), most of the morpho-functional attributes show a higher degree of variability than technological attributes, and no significant patterns could be clearly noticed. However, two attributes were relatively constant across all occupations: most damaged platforms present less than 45º angles; and, most pieces have bifacial scars on one damaged platform and unifacial on the other. On a closer inspection, we find that most of the differences within the morpho-functional variables occur between raw materials rather than between chronologies.
...
# Join attributes
m2 <- m2 %>% 
  mutate(Damage_Degree = str_c(Damage_Degree_PlatformA,Damage_Degree_PlatformB, sep = "/")) %>% 
  mutate(Scar_Shape = str_c(ScarShape_PlatformA,ScarShape_PlatformB, sep = "/")) %>%
  mutate(Scar_Distribution = str_c(ScarDistribution_PlatformA,ScarDistribution_PlatformA, sep = "/")) %>%
  mutate(Scar_Arrangement = str_c(ScarArrangement_PlatformA,ScarArrangement_PlatformB, sep = "/")) %>%
  mutate(Scar_Extension = str_c(ScarExtention_PlatformA,ScarExtention_PlatformB, sep = "/")) %>%
  mutate(Scar_FacialDistribution = str_c(ScarFacialDistribution_PlatformA,ScarFacialDistribution_PlatformB, sep = "/")) %>%
