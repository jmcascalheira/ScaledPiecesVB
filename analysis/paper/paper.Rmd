...
The patterns of use and rotation of damaged platforms seem to be the same across all chronologies. In every assemblage quartz pieces were exclusively used in one single axis, exhibiting only two damaged platforms. On the other end, some chert and chalcedony artifacts exhibit multiple functional axis, with three or four damaged platforms (Figure \@ref(fig:nplatforms)). This seems to indicate different strategies of curation for coarse and fine grain raw materials. However, when plotted against metric data, results for the Area variable, on the one hand, reveal that pieces with four damaged platforms are amongst the largest, and, on the other, there is not a visible difference between the thickness of the pieces comprising each group. <!-- why is this relevant? --> 
...
ggplot(n_axis, aes(x = factor(RawMaterial), y = perc, fill = factor(DamagedPlatforms))) +
  ...
  theme_bw() +
  theme(strip.text.x = element_text(size = 14), text = element_text(size=14))
...
With few exceptions, most of the morpho-functional attributes show a higher degree of variability than technological attributes. In fact, only two attributes were relatively constant across all occupations: most damaged platforms present less than 45º angles; and, a large percentage of pieces with two damaged platforms present bifacial scars on one of the platforms and unifacial scars on the opposite.

On a closer inspection, we find that most of the differences within the morpho-functional variables occur between raw materials rather than between chronologies.
